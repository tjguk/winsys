...
class _DropTarget (win32com.server.policy.DesignatedWrapPolicy):
  u"""Helper class to implement the IDropTarget interface so that
  ...
  def DragEnter (self, data_object, key_state, point, effect):
    u"""Query the data block for a drag action which is over the dialog.
  ...
  def DragOver (self, key_state, point, effect):
    u"""If there is a drag over one of the edit fields in the dialog
  ...
  def DragLeave (self):
    u"""Do nothing, but the method must be implemented.
    """
    pass
...
def _register_wndclass ():
  u"""Register a simple window with default cursor, icon, etc.
  """
...
class BaseDialog (object):
  u"""Basic template for a dialog with one or more fields plus
  ...
  def __init__ (self, title, parent_hwnd=0):
    u"""Initialise the dialog with a title and a list of fields of
  ...
  def _get_dialog_template (self):
    u"""Put together a sensible default layout for this dialog, taking
...
class Dialog (BaseDialog):
  u"""A general-purpose dialog class for collecting arbitrary information in
  ...
  def run (self):
    u"""The heart of the dialog box functionality. The call to DialogBoxIndirect
  ...
  def corners (self, l, t, r, b):
    u"""Designed to be subclassed (eg by :class:`InfoDialog`). By
    default simply returns the values unchanged.
    """
    return l, t, r, b

  def OnInitDialog (self, hwnd, msg, wparam, lparam):
    u"""Attempt to position the dialog box more or less in
  ...
  def _resize (self, dialog_w, dialog_h, repaint=1):
    u"""Attempt to resize the controls on the dialog, spreading
  ...
  def _get_item (self, item_id):
    u"""Return the current value of an item in the dialog.
    """
  ...
  def _set_item (self, item_id, value):
    u"""Set the current value of an item in the dialog
    """
  ...
  def OnMinMaxInfo (self, hwnd, msg, wparam, lparam):
    u"""Prevent the dialog from resizing vertically by extracting
  ...
  def _enable (self, id, allow=True):
    u"""Convenience function to enable or disable a control by id
    """
    wrapped (
  ...
  def OnProgressMessage (self, hwnd, msg, wparam, lparam):
    u"""Respond to a progress update from within the progress
  ...
  def OnProgressComplete (self, hwnd, msg, wparam, lparam):
    u"""Respond to the a message signalling that all processing is
  ...
  def _progress_complete (self, message):
    u"""Convenience function to tell the dialog that progress is complete,
  ...
  def _progress_message (self, message):
    u"""Convenience function to send progress messages to the dialog
    """
    _message = buffer (marshal.dumps (message))
    address, length = win32gui.PyGetBufferAddressAndLen (_message)
  ...
  def OnOk (self, hwnd):
    u"""When OK is pressed, if this isn't a progress dialog then simply
    ...
    def progress_thread (iterator, cancelled):
      u"""Handle the progress side of the dialog by iterating over a supplied
  ...
  def OnCancel (self, hwnd):
    u"""If the user presses cancel check to see whether we're running within
  ...
  def OnCallback (self, hwnd, field_id):
    u"""If the user pressed a callback button associated with a text
  ...
  def OnCommand (self, hwnd, msg, wparam, lparam):
    u"""Handle button presses: OK, Cancel and the callback buttons
