# -*- coding: utf-8 -*-
from __future__ import unicode_literalsu"""AsyncIO objects wrap the Win32 Overlapped API. They are instantiated by
...
class AsyncIO (core._WinSysObject):
  ...
  def pyobject (self):
    u"""Return the pyobject of the underlying event so that this object can
    be waited on by the :func:`ipc.all` or :func:`ipc.any` functions
    """
    return self.event.pyobject ()

  def is_complete (self):
    u":returns: `True` if the IO has completed"
    return self.event.isSet ()
  __nonzero__ = is_complete

  def wait (self):
    u"""Wait for the IO to complete in such a way that the wait can
...
class AsyncReader (AsyncHandler):
  ...
  def data (self):
    u"""Wait until the IO has completed and return the data from the read. This
